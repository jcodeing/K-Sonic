...
public class MediaPlayer implements IMediaPlayer {//extends AbstractMediaPlayer {
    /*--------------------
     * Listeners
     */
    ...
    public interface OnSpeedMediaCompletionListener {
        void onSpeedMediaCompletion(MediaPlayer arg0);
    }
...
//        return context.getPackageManager().queryIntentServices(new Intent(action), 65536).size() > 0;
    ...
    private static final String MP_TAG = "AoReplacementMediaPlaye";
    ...
    //加速框架切换完成
    OnSpeedMediaCompletionListener onSpeedMediaCompletionListener = null;
    ...
    private void switchMediaPlayerImpl(MediaPlayerImpl from, MediaPlayerImpl to) {
        ...
        try {
            ...
            if ((from == to)
                    ...
                    // Track in use MediaExtractor Call requires API level 16 (current min is 9)
...
//                    from.release();
            ...
            //OnSpeedMediaCompletionListener
            if (null != onSpeedMediaCompletionListener)
                onSpeedMediaCompletionListener.onSpeedMediaCompletion(this);

            Log.d(MP_TAG, "switchMediaPlayerImpl() 625 " + this.state.toString());
        } catch (Exception e) {
            if (null != onErrorListener) {
                onErrorListener.onError(null, -1010 * 2, 0);////MediaPlayer.MEDIA_ERROR_UNSUPPORTED //API:17
            }
        } finally {
            if (captured)
                lock.unlock();
    ...
    public void release() {
        ...
        try {
            ...
            this.onSpeedMediaCompletionListener = null;
    ...
    public float[] getVolume() {
        boolean captured = lock.tryLock();
        try {
            return new float[]{mLeftVolume, mRightVolume};
        } finally {
            if (captured)
                lock.unlock();
        }
    ...
    public void setOnSpeedMediaCompletionListener(OnSpeedMediaCompletionListener listener) {
        boolean captured = lock.tryLock();
        try {
            this.onSpeedMediaCompletionListener = listener;
        } finally {
            if (captured)
                lock.unlock();
        }
