 ...
 */
package com.jcodeing.library_exo.player;
...
public class DemoPlayer implements ExoPlayer.Listener, ChunkSampleSource.EventListener,
  ...
  /* package */ void onRenderersError(Exception e) {
    ...
    public final void setSonicSpeed(float speed) {
        if (audioRenderer instanceof SonicMediaCodecAudioTrackRenderer)
            ((SonicMediaCodecAudioTrackRenderer) audioRenderer).setSonicSpeed(speed);
    }

    public float getSonicSpeed() {
        if (audioRenderer instanceof SonicMediaCodecAudioTrackRenderer)
            return ((SonicMediaCodecAudioTrackRenderer) audioRenderer).getSonicSpeed();
        return 1.0f;
    }

    public final void setSonicPitch(float pitch) {
        if (audioRenderer instanceof SonicMediaCodecAudioTrackRenderer)
            ((SonicMediaCodecAudioTrackRenderer) audioRenderer).setSonicPitch(pitch);
    }

    public float getSonicPitch() {
        if (audioRenderer instanceof SonicMediaCodecAudioTrackRenderer)
            return ((SonicMediaCodecAudioTrackRenderer) audioRenderer).getSonicPitch();
        return 1.0f;
    }

    public final void setSonicRate(float rate) {
        if (audioRenderer instanceof SonicMediaCodecAudioTrackRenderer)
            ((SonicMediaCodecAudioTrackRenderer) audioRenderer).setSonicRate(rate);
    }

    public float getSonicRate() {
        if (audioRenderer instanceof SonicMediaCodecAudioTrackRenderer)
            return ((SonicMediaCodecAudioTrackRenderer) audioRenderer).getSonicRate();
        return 1.0f;
    }
}
